...
template <class T> struct managed_allocator {
  ...
  template <class U>
  constexpr managed_allocator(const managed_allocator<U> &) {}
  ...
  T *allocate(std::size_t n, cudaStream_t stream = 0) const {
    ...
    if (error != cudaSuccess) {
      ...
      std::runtime_error(
          "cudaMalloc failed in the hash_allocator.cuh:managed_allocator.");
    }
    return d_tmp;
...
template <class T> struct default_allocator {
  ...
  T *allocate(std::size_t n, cudaStream_t stream = 0) const {
    ...
    if (error != cudaSuccess) {
      cudaGetLastError(); // clear CUDA error
      std::runtime_error(
          "cudaMalloc failed in the hash_allocator.cuh:default_allocator.");
    }
